...
const useUserAcceptanceValidation = () => {
	...
	const [selectedScenario, setSelectedScenario] = useState<UserScenario | null>(
		null
	);
			...
			const mockScenarios: UserScenario[] = [
				{
					...
					description:
						"Flujo completo desde cálculo inicial hasta cronograma detallado para proyecto residencial",
					userType: "architect",
					...
					steps: [
						{
							...
							description:
								"Usuario inicia sesión y accede al dashboard principal",
							userAction: 'Ingresar credenciales y hacer clic en "Ingresar"',
							expectedResult:
								"Dashboard principal cargado con proyectos del usuario",
							status: "passed",
							timeSpent: 2,
						},
						{
							...
							userAction:
								'Hacer clic en "Nuevo Proyecto" y completar formulario',
							expectedResult:
								"Proyecto creado y usuario redirigido a vista de proyecto",
							status: "passed",
							timeSpent: 5,
						},
						{
							...
							userAction:
								"Navegar a catálogo de cálculos y seleccionar template",
						...
						},
						{
							...
							expectedResult:
								"Resultados generados con cantidades de materiales",
							status: "failed",
					...
					acceptanceCriteria: [
						{
							id: "ac1",
							criterion:
								"El usuario puede completar el flujo completo en menos de 60 minutos",
							priority: "must_have",
							status: "failed",
						...
						},
						{
							id: "ac2",
							criterion:
								"Todos los cálculos deben ser precisos según normativa NEC",
							priority: "must_have",
							status: "not_tested",
						},
						{
							id: "ac3",
							criterion:
								"La interfaz debe ser intuitiva para arquitectos sin experiencia técnica",
							...
							issues: [
								{
									...
									description:
										"El error en validación de parámetros sísmicos no explica claramente qué debe corregir el usuario",
									stepId: "1.4",
									reproduced: true,
									status: "open",
								},
								{
									...
									description:
										"El catálogo de templates tarda más de 5 segundos en cargar",
									stepId: "1.3",
									reproduced: true,
									status: "in_progress",
								},
							...
							suggestions: [
								{
									...
									description:
										"Incluir explicaciones breves para parámetros técnicos complejos",
									priority: "medium",
									votes: 3,
									status: "approved",
								},
					...
					feedback: [
						{
							...
							rating: 4,
							comment:
								"En general muy buena herramienta, pero necesita mejorar la claridad de los mensajes de error.",
				...
				},
				{
					...
					description:
						"Uso de la aplicación móvil para reportar avances desde la obra",
					userType: "contractor",
					complexity: "basic",
					estimatedTime: 20,
					status: "completed",
					...
					steps: [
						{
							...
							description:
								"Acceder a la aplicación desde dispositivo móvil en obra",
					...
					acceptanceCriteria: [
						...
						},
						{
							id: "ac5",
							criterion:
								"Los reportes deben sincronizarse automáticamente cuando hay conexión",
							priority: "must_have",
							status: "passed",
							testDate: new Date(2024, 5, 11),
							tester: "Contratista Beta",
							...
							suggestions: [
								{
									...
									description:
										"Permitir grabación de notas de voz para reportes",
									priority: "low",
									votes: 2,
									status: "under_review",
								},
		...
		<div className="bg-white rounded-xl border border-gray-200 p-4">
			<div className="flex items-center gap-3">
				...
				<div>
					...
					<div className="text-xl font-bold text-gray-900">
						{(
							(metrics.passedScenarios / metrics.completedScenarios) *
							100
						).toFixed(1)}
						%
					</div>
...
const ScenarioCard: React.FC<{
	...
	const passedSteps = scenario.steps.filter(
		(s) => s.status === "passed"
	).length;
				...
				<div className="mb-4">
					<div className="flex items-center gap-2 mb-2">
						...
						<span className="text-sm text-gray-600">
							({scenario.testResults.length} test
							{scenario.testResults.length !== 1 ? "s" : ""})
						</span>
					</div>
				</div>
			)}
...
const ScenarioDetailsModal: React.FC<{
	...
	const [activeTab, setActiveTab] = useState<
		"steps" | "criteria" | "results" | "feedback"
	>("steps");
								...
								<div key={criterion.id} className="bg-gray-50 rounded-lg p-4">
									<div className="flex items-start justify-between mb-2">
										<div className="flex-1">
											...
											<div className="flex items-center gap-2 mt-2">
												<Badge
													variant={
														criterion.priority === "must_have"
															? "danger"
															: criterion.priority === "should_have"
																? "warning"
																: "secondary"
													}
												>
									...
									{criterion.testDate && (
										<div className="text-xs text-gray-500 mt-2">
											Probado: {criterion.testDate.toLocaleDateString()} por{" "}
											{criterion.tester}
										</div>
									)}
