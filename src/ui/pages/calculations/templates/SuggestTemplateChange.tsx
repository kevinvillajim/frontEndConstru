...
const mockTemplate: PublicCalculationTemplate = {
	...
	description:
		"Plantilla para el cálculo de dimensiones de escaleras según NEC",
	longDescription:
		"Cálculo completo que incluye huella, contrahuella, ancho y altura total de escaleras residenciales",
...
const SuggestTemplateChange: React.FC<SuggestTemplateChangeProps> = ({
	...
	// CORREGIDO: Agregar tipado al useParams
	const {templateId} = useParams<{templateId: string}>();

	// Hook para envío de sugerencias
	const {submitSuggestion} = useTemplates({autoLoad: false});

	// AGREGADO: Estado para el template actual
	const [currentTemplate, setCurrentTemplate] =
		useState<PublicCalculationTemplate>(template || mockTemplate);

	// AGREGADO: Efecto para cargar template por ID si no viene como prop
	useEffect(() => {
		if (templateId && !template) {
			// Aquí deberías cargar el template por ID desde la API
			// loadTemplateById(templateId).then(setCurrentTemplate);
			console.log("Loading template by ID:", templateId);

			// Por ahora usar el mock template pero con el ID correcto
			setCurrentTemplate({
				...mockTemplate,
				id: templateId,
			});
	...
	const selectedType = suggestionTypes.find(
		(type) => type.id === formData.suggestionType
	);
			...
			<div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
				<div className="max-w-md w-full bg-white rounded-2xl shadow-lg p-8 text-center">
					...
					<p className="text-gray-600 mb-6">
						Tu sugerencia ha sido enviada exitosamente. Nuestro equipo técnico
						la revisará y te contactaremos si es necesario.
					</p>
					...
					<div className="flex items-start gap-4">
						...
						<div className="flex-1">
							...
							<p className="text-gray-600 mb-4">
								{/* CORREGIDO: Usar currentTemplate en lugar de template */}
								Ayúdanos a mejorar la plantilla "{currentTemplate.name}" con tus
							...
							<h2 className="text-xl font-semibold text-gray-900 mb-6 flex items-center gap-2">
								{selectedType?.icon &&
									React.createElement(selectedType.icon, {
										className: `h-5 w-5 ${selectedType.color}`,
									})}
									...
									<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
										...
										<div>
											...
											<textarea
												...
												onChange={(e) =>
													handleInputChange("proposedValue", e.target.value)
												}
												rows={4}
									...
									<div className="space-y-3">
										<div className="flex items-start gap-3 p-4 border border-gray-200 rounded-lg">
											<input
												...
												onChange={(e) =>
													handleInputChange("affectsAccuracy", e.target.checked)
												}
												className="mt-1 h-4 w-4 text-primary-600 focus:ring-primary-500 border-gray-300 rounded"
											/>
											<div>
												<label
													htmlFor="affectsAccuracy"
													className="text-sm font-medium text-gray-900"
												>
													Precisión del cálculo
												</label>
												<p className="text-sm text-gray-600 mt-1">
													Esta sugerencia corrige o mejora la precisión de los
													resultados
												</p>
											</div>
										</div>
										...
										<div className="flex items-start gap-3 p-4 border border-gray-200 rounded-lg">
											...
											<div>
												<label
													htmlFor="affectsCompliance"
													className="text-sm font-medium text-gray-900"
												>
													Cumplimiento normativo
												</label>
												<p className="text-sm text-gray-600 mt-1">
													Esta sugerencia está relacionada con requisitos del
													NEC u otras normas
												</p>
											</div>
										</div>
							...
							<div className="space-y-4">
								{formData.references.length === 0 ? (
									<div className="text-center py-8 text-gray-500">
										...
										<p className="text-sm">
											Las referencias ayudan a validar tu sugerencia pero son
											opcionales
										</p>
									</div>
								) : (
								...
								<div className="border-t border-gray-200 pt-6">
									<div className="flex items-start gap-3">
										...
										<div>
											...
											<p className="text-sm text-gray-600 mt-1">
												Nuestro equipo técnico podrá contactarte para aclarar
												detalles o notificarte sobre el estado de tu sugerencia
											</p>
										</div>
									</div>
								</div>
										...
										<div className="flex gap-3">
											...
											<div>
												...
												<p className="text-sm text-yellow-800">
													Has marcado que esta sugerencia afecta la precisión o
													el cumplimiento normativo. Será revisada con especial
													atención por nuestros expertos técnicos.
												</p>
											</div>
										</div>
									</div>
					...
					<div className="flex gap-3">
						...
						<div>
							...
							<p className="text-sm text-blue-800">
								Tu sugerencia será revisada por nuestro equipo técnico y
								expertos en normativa NEC. Te notificaremos sobre el estado de
								tu propuesta y su posible implementación en futuras versiones de
								la plantilla.
							</p>
						</div>
					</div>
