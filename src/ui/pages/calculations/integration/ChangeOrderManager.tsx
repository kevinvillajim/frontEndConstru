		...
		<div className="bg-white rounded-xl border border-gray-200 p-6 hover:shadow-lg transition-all duration-300">
			<div className="flex items-start justify-between mb-4">
				<div className="flex items-start gap-3 flex-1">
					<div
						className={`w-1 h-16 rounded-full ${getPriorityColor(changeOrder.priority)}`}
					/>
			...
			<div className="grid grid-cols-2 gap-4 mb-4">
				<div className="bg-gray-50 rounded-lg p-3">
					...
					>
						{changeOrder.budgetImpact > 0 ? "+" : ""}
						{formatCurrency(changeOrder.budgetImpact)}
					</div>
				</div>
				...
				<div className="bg-gray-50 rounded-lg p-3">
					...
					>
						{changeOrder.scheduleImpact > 0 ? "+" : ""}
						{changeOrder.scheduleImpact} días
					</div>
				</div>
			</div>
					...
					<div className="flex items-center gap-2">
						<div
							className={`px-2 py-1 rounded text-xs font-medium ${
								changeOrder.riskAssessment.overall === "critical"
									? "bg-red-100 text-red-800"
									: changeOrder.riskAssessment.overall === "high"
										? "bg-orange-100 text-orange-800"
										: changeOrder.riskAssessment.overall === "medium"
						...
						<span className="text-xs text-gray-500">
							T:{changeOrder.riskAssessment.factors.technical}/ F:
							{changeOrder.riskAssessment.factors.financial}/ S:
							{changeOrder.riskAssessment.factors.schedule}/ Q:
							{changeOrder.riskAssessment.factors.quality}
						</span>
					</div>
				</div>
			)}
...
const ChangeOrderManager: React.FC = () => {
	...
	const pendingApprovals = changeOrders.filter(
			...
			<div className="bg-white border-b border-gray-200 p-6">
					...
					<div className="flex items-center gap-4">
						...
						<div>
							...
							<p className="text-gray-600">
								Administra cambios al proyecto con impacto automático
							</p>
						</div>
					</div>
					<button
				...
				<div className="grid grid-cols-1 md:grid-cols-4 gap-4">
					...
					<div className="bg-purple-50 rounded-lg p-4">
						...
						<div
							className={`text-2xl font-bold ${totalBudgetImpact >= 0 ? "text-red-900" : "text-green-900"}`}
						>
							{totalBudgetImpact >= 0 ? "+" : ""}$
				...
				{filteredChangeOrders.length === 0 ? (
					<div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
						...
						<p className="text-gray-600 mb-6">
							Crea la primera orden de cambio para gestionar modificaciones al
							proyecto
						</p>
						<button
					...
					<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
						{filteredChangeOrders.map((changeOrder) => (
							<ChangeOrderCard
								...
								onApprove={(id) =>
									approveChangeOrder(id, "Aprobado desde gestor")
								}
								onReject={(id) =>
									rejectChangeOrder(id, "Rechazado desde gestor")
								}
